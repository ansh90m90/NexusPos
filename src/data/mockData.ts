...
export const createInitialAccountState = (id: string, name: string, shopTypes: ShopType[]): AccountState => {
    ...
    return {
        ...
        expenses: [],
        heldCarts: [],
    } as any; // Cast to any to avoid missing fields in mock data for now
};
...
export const createTestAccountState = (): AccountState => {
    ...
    dishNames.forEach((name, i) => {
        state.dishes.push({
            id: i + 1, name, price: randInt(50, 150), imageUrl: `https://picsum.photos/seed/${name.replace(' ','')}/400/300`,
    ...
    for(let i=0; i<50; i++) {
        ...
        if (customer && t.payments[0].method === 'Credit') {
            const c: any = state.customers.find(c => c.id === customer.id);
