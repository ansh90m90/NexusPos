...
import type { AppSettings, EmployeeRole, User, Promotion, PromotionType, Product, PromotionConditionTarget, AccountState, Theme, HistoryLog, ItemType, StockAdjustment, UiScale, AccentColor } from '../types';
  ...
  <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-900/50 p-4 rounded-lg">
    ...
    <label htmlFor={`toggle-${title.replace(/\s+/g, '-')}`} className={`relative inline-flex items-center ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
      <input 
        ...
        disabled={disabled}
      />
      <div className={`w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary-300 dark:peer-focus:ring-primary-800 rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-slate-600 peer-checked:bg-primary-600 ${disabled ? 'opacity-50' : ''}`}></div>
    </label>
  </div>
);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[70] p-4 modal-content">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 max-w-md w-full space-y-4">
                <h3 className="text-xl font-bold">{isEditing ? 'Edit Staff Member' : 'Add New Staff'}</h3>
                <input name="name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Full Name" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600 focus:ring-1 focus:ring-primary-500" required />
                <input name="email" type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="Email Address" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600 focus:ring-1 focus:ring-primary-500" required />
                <input name="password" type="password" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} placeholder={isEditing ? 'New Password (leave blank to keep current)' : 'Password'} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600 focus:ring-1 focus:ring-primary-500" required={!isEditing} />
                <select name="role" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as EmployeeRole})} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600">
...
const PromotionModal: React.FC<{
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[70] p-4 modal-content">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col">
                <div className="p-4 border-b dark:border-slate-700"><h3 className="text-xl font-bold">{isEditing ? 'Edit Promotion' : 'Add New Promotion'}</h3></div>
                <div className="p-6 space-y-4 overflow-y-auto">
                    <input name="name" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="Promotion Name (e.g., Weekend Sale)" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" required />
                    <div className="grid grid-cols-2 gap-4">
                        <select name="type" value={formData.type} onChange={e => setFormData({...formData, type: e.target.value as PromotionType})} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600"><option value="PERCENTAGE_OFF">Percentage Off</option><option value="FIXED_AMOUNT_OFF">Fixed Amount Off</option></select>
                        <input type="number" name="value" value={formData.value} onChange={e => setFormData({...formData, value: parseFloat(e.target.value) || 0})} placeholder={formData.type === 'PERCENTAGE_OFF' ? '% value' : '₹ value'} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" required />
                    </div>
                    <h4 className="font-semibold pt-2 text-sm border-t dark:border-slate-700">Conditions</h4>
                    <div className="space-y-3">
                        <div><label className="text-xs font-medium">Minimum Purchase (₹)</label><input type="number" value={formData.conditions?.minPurchase || ''} onChange={e => handleConditionChange('minPurchase', parseFloat(e.target.value) || 0)} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" placeholder="0 for no minimum" /></div>
                        <div>
                            <label className="text-xs font-medium">Applies To</label>
                            <select value={formData.conditions?.appliesTo} onChange={e => handleConditionChange('appliesTo', e.target.value as PromotionConditionTarget)} className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600">
                        ...
                        {formData.conditions?.appliesTo === 'SPECIFIC_PRODUCTS' && (
                            <div className="max-h-40 overflow-y-auto p-2 border rounded dark:border-gray-600">
                                {products.flatMap(p => p.variants).map(v => (
                                    <label key={v.id} className="flex items-center gap-2 text-sm p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        {formData.conditions?.appliesTo === 'SPECIFIC_CATEGORIES' && (
                            <div className="max-h-40 overflow-y-auto p-2 border rounded dark:border-gray-600">
                                {allSubCategories.map(cat => (
                                     <label key={cat} className="flex items-center gap-2 text-sm p-1 rounded hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <div className="p-4 border-t dark:border-slate-700 mt-auto bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-4">
                     <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg bg-slate-200 dark:bg-slate-600 hover:bg-slate-300 dark:hover:bg-slate-500 transition">Cancel</button>
                     <button type="submit" className="px-4 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition">Save</button>
                </div>
            </form>
...
const Settings: React.FC<SettingsProps> = ({ accountState, setAppSettings, onSaveUser, onDeleteUser, onSavePromotion, onDeletePromotion, onTogglePromotionStatus, onHardReset, onRestoreItem, uiScale, setUiScale, isTutorialActive, onStartTutorial, onEndTutorial, modalState, setModalState }) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'general':
                return (
                    <div className="space-y-4">
                        <input value={appSettings.shopName} onChange={e => handleAppSettingsChange('shopName', e.target.value)} placeholder="Shop Name" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" />
                        <input value={appSettings.upiId} onChange={e => handleAppSettingsChange('upiId', e.target.value)} placeholder="UPI ID (for QR codes)" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" />
                        <input value={appSettings.receiptFooter} onChange={e => handleAppSettingsChange('receiptFooter', e.target.value)} placeholder="Receipt Footer Message" className="w-full p-2 border rounded bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600" />
                    </div>
                );
            case 'appearance': {
                ...
                return (
                     <div className="space-y-6">
                        ...
                        <div className="bg-slate-100 dark:bg-slate-900/50 p-4 rounded-lg">
                            <h3 className="font-semibold text-slate-800 dark:text-white mb-2">Accent Color</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Choose the primary color for the application.</p>
                         ...
                         <div className="bg-slate-100 dark:bg-slate-900/50 p-4 rounded-lg">
                            <h3 className="font-semibold text-slate-800 dark:text-white mb-2">UI Scale</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Adjust the size of the interface elements.</p>
                                    ...
                                    <button
                                        ...
                                        className={`flex-1 py-2 px-4 rounded-lg border text-sm font-medium transition-all ${
                                            uiScale === scale
                                                ...
                                                : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-primary-500'
                                        }`}
                                    >
                ...
                return (
                    <div>
                        <Tooltip content="Add a new staff member" position="bottom">
                            <button onClick={() => setModalState({ type: 'add_user', data: null })} className="mb-4 px-4 py-2 text-sm font-semibold rounded-lg bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300 hover:bg-primary-200 dark:hover:bg-primary-900 transition">Add Staff</button>
                        </Tooltip>
                        <div className="space-y-2">
                           {users.map(user => (
                               <div key={user.id} className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                 ...
                 return (
                    <div>
                        <Tooltip content="Create a new promotion" position="bottom">
                            <button onClick={() => setModalState({ type: 'add_promo', data: null })} className="mb-4 px-4 py-2 text-sm font-semibold rounded-lg bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300 hover:bg-primary-200 dark:hover:bg-primary-900 transition">Add Promotion</button>
                        </Tooltip>
                        <div className="space-y-2">
                            {promotions.map(promo => (
                                <div key={promo.id} className="flex justify-between items-center p-3 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
                                    <div>
                                        <p className="font-semibold">{promo.name}</p>
                                        <p className="text-sm text-slate-500">{promo.type === 'PERCENTAGE_OFF' ? `${promo.value}% off` : `₹${promo.value} off`}</p>
                                    </div>
            ...
            case 'history': return <div>{history.slice(-20).reverse().map(h => <div key={h.id} className="text-sm p-2 border-b dark:border-slate-700">{h.timestamp}: {h.user} {h.action}d {h.itemType} "{h.itemName}"</div>)}</div>;
            case 'adjustments': return <div>{stockAdjustments.slice(-20).reverse().map(a => <div key={a.id} className="text-sm p-2 border-b dark:border-slate-700">{a.timestamp}: {a.user} adjusted stock for {a.productName} by {a.quantityChange} ({a.reason})</div>)}</div>;
            case 'data': return (
                <div className="space-y-4">
                    <button onClick={() => setModalState({type: 'data_transfer', data: null})} className="w-full text-left p-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-900/50 dark:hover:bg-slate-800">
                        <h3 className="font-semibold">Import/Export Data</h3>
                        <p className="text-sm text-slate-500">Transfer your shop data between devices.</p>
                    </button>
                    ...
                    <div>
                        ...
                        {deletedItems.map(item => (
                            <div key={`${item.itemType}-${item.id}`} className="flex justify-between items-center text-sm p-2 bg-slate-100 dark:bg-slate-900/50 rounded-md mb-1">
            ...
            case 'help': return (
                <div className="space-y-4">
                    <button onClick={isTutorialActive ? onEndTutorial : onStartTutorial} className="w-full text-left p-4 rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-900/50 dark:hover:bg-slate-800">
                        <h3 className="font-semibold">{isTutorialActive ? 'End Tutorial' : 'Restart Welcome Tutorial'}</h3>
                        <p className="text-sm text-slate-500">Get a guided tour of the application's main features.</p>
                    </button>
                </div>
            );
            default: return null;
                        ...
                        {TABS.map(tab => (
                            <li key={tab.id}>
                                <Tooltip content={`Go to ${tab.label} settings`} position="right">
                                    <button onClick={() => setActiveTab(tab.id)} className={`w-full text-left flex items-center gap-3 px-3 py-2 rounded-md font-semibold text-sm transition-colors ${activeTab === tab.id ? 'bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-700'}`}>
